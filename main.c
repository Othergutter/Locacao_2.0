...
void displayUser (struct userData * userInfo); // imprime os campos dos usuários
...
void displayUser (struct userData * userInfo) {
        printf("\nNome: %s\n", userInfo->nameUser);
        printf("\nMat: %s\n", userInfo->userMat);
}
...
struct userData * findUserMat (char * userMat) {
     ...
     userInfo = userStart;
     while(userInfo) {
        if (!strcmp(userMat, userInfo->userMat)) return userInfo;
        userInfo = userInfo->next; // novo endereço
     }
...
void registerUser () {
     ...
     for(;;){
        ...
        accurateInput("Insira a matricula (nesse padrao '001', '002'): ", userInfo->userMat, 4);
